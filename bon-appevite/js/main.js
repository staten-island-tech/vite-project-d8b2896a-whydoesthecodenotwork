...
function insertCard(product) {
	...
	if (
		...
		// add a select element if it doesn't already exist
		...
		card.querySelector("select").addEventListener(
			"onchange",
...
function updateCard(card, product) {
	if (card.querySelector("select") !== null) {
		var item = product.types[card.querySelector("select").selectedIndex];
		card.innerHTML = `
							<h2>${product.name}</h2>
							<s><h4>$${item.price}</h4></s>
								<h3>$${item.discounted}</h3>
